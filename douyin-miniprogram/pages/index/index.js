...
Page({
  ...
  onLoad(options) {
    ...
    if (eid > 0) {
      ...
      tt.navigateTo({ url: '/pages/enterprise/index?uid=' + uid + '&eid=' + eid })
      return
    }
  ...
  onShow() {
    ...
    const token = getApp().globalData.token || tt.getStorageSync('token')
    if (token) {
      request({
        url: '/api/auth/me',
        method: 'GET',
  ...
  switchToEnterprise() {
    ...
    app.ensureLogin().then(() => app.getRuntimeConfig()).then((cfg) => {
      ...
      tt.showToast({ title: '无法获取配置，请稍后重试', icon: 'none' })
    })
  },
